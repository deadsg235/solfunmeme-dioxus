...
use md5; // Keep md5 as it's explicitly requested for this crate
use anyhow::Result; // Added for error handling
...
pub fn extract_code_snippets(content: &str) -> Vec<CodeSnippet> {
        ...
        if line.starts_with("```") {
            if in_code_block {
                ...
                if !current_content.trim().is_empty() {
                    let snippet = CodeSnippet { // Direct creation
                        language: current_language.clone(),
                        content: current_content.clone(),
                        line_start: start_line as u32,
                        line_end: i as u32,
                        content_hash: format!("{:x}", md5::compute(&current_content)), // Example hash
                        token_count: current_content.len() / 5, // Placeholder
                        line_count: (i - start_line) as u32,
                        char_count: current_content.len() as u32,
                        test_result: None,
                    };
    ...
    if in_code_block && !current_content.trim().is_empty() {
        let snippet = CodeSnippet { // Direct creation
            ...
            content_hash: format!("{:x}", md5::compute(&current_content)), // Example hash
            token_count: current_content.len() / 5, // Placeholder
            line_count: (lines.len() - start_line) as u32,
            char_count: current_content.len() as u32,
            test_result: None,
        };
        snippets.push(snippet);
