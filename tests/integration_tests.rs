...
fn test_full_code_analysis_pipeline() {
    ...
    let analysis = analyzer.analyze_file(SAMPLE_RUST_CODE, "sample.rs".to_string())
...
"#;
    ...
    let mut analyzer = CodeAnalyzer::new(64, 0.8);
    let simple_analysis = analyzer.analyze_file(simple_code, "simple.rs".to_string()).unwrap();
    let complex_analysis = analyzer.analyze_file(complex_code, "complex.rs".to_string()).unwrap();
...
"#;
    ...
    let analysis = analyzer.analyze_file(code_with_duplicates, "test.rs".to_string()).unwrap();
