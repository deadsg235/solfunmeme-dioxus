use dioxus::prelude::*;
use crate::model::wasm_bert::{WasmBertEmbedder, WasmSentimentAnalyzer, WasmTextClassifier, WasmTextSummarizer};
...
pub fn BertTestApp() -> Element {
    ...
    let mut process_text = move |text: &str| {
        ...
        for (i, &val) in embedding.iter().take(10).enumerate() {
            ...
            if i < 9 { result_text.push_str(", "); }
        }
        result_text.push_str("\n\n");
        
        result_text.push_str(&format!("😊 Sentiment Score: {:.4} ({})\n\n", 
            sentiment, 
            if sentiment > 0.1 { "Positive" } else if sentiment < -0.1 { "Negative" } else { "Neutral" }
        ));
