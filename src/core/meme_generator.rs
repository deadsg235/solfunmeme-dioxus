use crate::core::code_analyzer::CodeAnalysis;
use crate::core::declaration_splitter::{Declaration, DeclarationType};
...
impl MemeGenerator {
    ...
    pub fn generate_meme_from_declaration(&self, declaration: &Declaration, vector: &CodeVector) -> Meme {
        let id = self.generate_meme_id(declaration);
        let emoji = self.emoji_map.get(&declaration.declaration_type)
            .cloned()
        ...
        let metadata = MemeMetadata {
            ...
            file_path: declaration.file_path.clone().unwrap_or_else(|| "unknown".to_string()),
        ...
        for analysis in analyses {
            for (i, declaration) in analysis.declarations.iter().enumerate() {
                if i < analysis.vectors.len() {
                    let meme = self.generate_meme_from_declaration(declaration, &analysis.vectors[i]);
    ...
    fn generate_meme_id(&self, declaration: &Declaration) -> String {
        let file_name = declaration.file_path
            .as_ref()
            .and_then(|p| std::path::Path::new(p).file_stem())
            .and_then(|s| s.to_str())
            .unwrap_or("unknown");
        
        format!("{}::{}::{}", file_name, declaration.declaration_type.to_string(), declaration.name)
    }
    ...
    fn calculate_emergence_level(&self, content: &str) -> u8 {
        ...
        if content.contains("impl") { level += 1; }
        if content.contains("trait") { level += 1; }
        if content.contains("generic") || content.contains("<") { level += 1; }
        if content.contains("macro") { level += 2; }
        if content.contains("unsafe") { level += 1; }
    ...
    fn calculate_semantic_coherence(&self, content: &str) -> f32 {
        let keywords = ["fn", "struct", "enum", "impl", "trait", "let", "mut", "if", "else", "match"];
    ...
    fn detect_relationships(&self, relationships: &mut HashMap<String, Vec<String>>, memes: &HashMap<String, Meme>) {
...
mod tests {
    ...
    fn test_complexity_calculation() {
        let generator = MemeGenerator::new(100);
        let complex_content = "fn complex() { if true { for i in 0..10 { match i { 0 => {}, _ => {} } } } }";
    ...
    fn test_emergence_level() {
        ...
        let complex_content = "impl<T> trait MyTrait for T where T: Clone { unsafe fn complex() {} }";
